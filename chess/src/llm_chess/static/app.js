...
const elements = {
  ...
  playersSection: document.querySelector("#player-section"),
  whitePlayerRow: document.querySelector("#white-player-row"),
  blackPlayerRow: document.querySelector("#black-player-row"),
  ...
  setupDescription: document.querySelector(".setup-copy"),
  colorChoice: document.querySelector("#human-color-choice"),
  cliPlayerForm: document.querySelector("#cli-player-form"),
  cliWhiteInput: document.querySelector("#white-player-input"),
  cliBlackInput: document.querySelector("#black-player-input"),
  cliStartButton: document.querySelector("#cli-start-button"),
...
function errorText() {
  ...
  const values = { ...ui.error.values };
  if (values.actorKey) {
    values.actor = i18n.t(values.actorKey);
    delete values.actorKey;
  }
  if (values.colorKey) {
    values.color = i18n.t(values.colorKey);
    delete values.colorKey;
  }
...
/** @description 현재 대국이 양쪽 CLI 참가자 모드인지 반환합니다. */
function _isCliVsCli() {
...
/** @description 스냅샷의 참가자 색상에 대응하는 표시 이름을 반환합니다. */
function _playerName(color) {
  const name = ui.snapshot.players[color].name;
  return name === null
    ? colorName(color)
    : i18n.t("player.nameWithColor", { color: colorName(color), name });
}

/** @description 기존 행위자 또는 CLI 참가자를 현재 언어로 표시합니다. */
function _gameActorName(actor) {
  return _isCliVsCli() ? _playerName(actor) : actorName(actor);
}
...
function resultText(result) {
  if (result === "1-0" || result === "0-1") {
    ...
    if (_isCliVsCli()) {
      return i18n.t("result.playerWin", { player: _playerName(winnerColor), result });
    }
    return winnerColor === ui.snapshot.human_color
      ? i18n.t("result.humanWin")
      : i18n.t("result.llmWin");
  }
  if (_isCliVsCli() && result === "1/2-1/2") {
    return i18n.t("result.draw", { result });
  }
...
function statusText() {
  ...
  if (snapshot.status === "resigned") {
    return i18n.t("status.resigned", {
      actor: _gameActorName(snapshot.resigned_by),
  ...
  if (isTakebackPending()) {
    if (_isCliVsCli()) {
      ...
      const opponent = requester === "white" ? "black" : "white";
      return i18n.t("status.cliTakeback", {
        player: _playerName(requester),
        opponent: _playerName(opponent),
      });
    }
    return i18n.t(snapshot.takeback.requester === "llm" ? "status.takebackLlm" : "status.takebackHuman");
  }
  if (snapshot.status === "checkmate") {
    return _isCliVsCli() ? resultText(snapshot.result) : statusReasonText();
  }
  if (snapshot.status === "draw") {
    return _isCliVsCli() ? resultText(snapshot.result) : statusReasonText();
  }
  if (_isCliVsCli()) {
    return i18n.t(snapshot.check ? "status.cliCheck" : "status.cliTurn", {
      player: _playerName(snapshot.turn),
    });
  }
  if (snapshot.check) {
...
function renderClock() {
  ...
  if (ui.connection === "error") {
    ...
    if (_isCliVsCli()) {
      ...
      const opponent = requester === "white" ? "black" : "white";
      tone = "llm";
      label = i18n.t("clock.cliTakeback.label");
      value = i18n.t("clock.cliTakeback.value");
      detail = i18n.t("clock.cliTakeback.detail", {
        player: _playerName(requester),
        opponent: _playerName(opponent),
  ...
  } else if (snapshot.game_id !== null && _isCliVsCli()) {
    tone = "human";
    label = i18n.t("clock.cliTurn.label");
    value = colorName(snapshot.turn);
    detail = i18n.t("clock.cliTurn.detail", { player: _playerName(snapshot.turn) });
...
/** @description CLI 대국 참가자와 현재 차례를 목록에 표시합니다. */
function _renderPlayers() {
  const cliGame = _isCliVsCli() && ui.snapshot.game_id !== null;
  elements.playersSection.hidden = !cliGame;
  if (!cliGame) {
    return;
  }
  elements.whitePlayerName.textContent = _playerName("white");
  elements.blackPlayerName.textContent = _playerName("black");
  elements.whitePlayerRow.classList.toggle("is-active", ui.snapshot.status === "active" && ui.snapshot.turn === "white");
  elements.blackPlayerRow.classList.toggle("is-active", ui.snapshot.status === "active" && ui.snapshot.turn === "black");
}
...
function render() {
  ...
  const cliSetup = ui.setupMode === "cli_vs_cli";
  const displayedMode = ui.setupOpen ? ui.setupMode : snapshot.game_mode;
  elements.appShell.dataset.status = snapshot.status;
  elements.brandTagline.textContent = i18n.t(displayedMode === "cli_vs_cli" ? "brand.tagline.cli" : "brand.tagline");
  elements.setupHeading.textContent = i18n.t(cliSetup ? "setup.cli.heading" : "setup.heading");
  elements.setupDescription.textContent = i18n.t(cliSetup ? "setup.cli.description" : "setup.description");
  elements.colorChoice.hidden = cliSetup;
  elements.cliPlayerForm.hidden = !cliSetup;
  elements.modeButtons.forEach((button) => {
    const selected = button.dataset.gameMode === ui.setupMode;
    button.setAttribute("aria-pressed", String(selected));
    button.disabled = ui.requestPending;
  });
  elements.cliStartButton.disabled = ui.requestPending;
  elements.cliWhiteInput.disabled = ui.requestPending;
  elements.cliBlackInput.disabled = ui.requestPending;
  ...
  if (!snapshot.game_id) {
    ...
    elements.boardCaption.textContent = i18n.t("board.caption.resigned", {
      actor: _gameActorName(snapshot.resigned_by),
      result: resultText(snapshot.result),
    });
  } else if (isTakebackPending() && _isCliVsCli()) {
    const requester = snapshot.takeback.requester;
    const opponent = requester === "white" ? "black" : "white";
    elements.boardCaption.textContent = i18n.t("board.caption.cliTakeback", {
      player: _playerName(requester),
      opponent: _playerName(opponent),
    });
  } else if (isTakebackPending()) {
    elements.boardCaption.textContent = i18n.t(snapshot.takeback.requester === "llm" ? "board.caption.takebackLlm" : "board.caption.takebackHuman", {
      color: colorName(snapshot.human_color),
    });
  } else if (_isCliVsCli()) {
    elements.boardCaption.textContent = snapshot.turn
      ? i18n.t("status.cliTurn", { player: _playerName(snapshot.turn) })
      : resultText(snapshot.result);
  } else {
    elements.boardCaption.textContent = i18n.t(snapshot.turn === "human" ? "board.caption.human" : snapshot.turn === "llm" ? "board.caption.llm" : "board.caption.terminal", {
      color: colorName(snapshot.human_color),
    });
  }
  ...
  elements.takebackButton.hidden = _isCliVsCli();
  elements.resignButton.hidden = _isCliVsCli();
  ...
  if (ui.setupOpen && setupWasHidden) {
    requestAnimationFrame(() => elements.modeButtons[0].focus());
...
function renderActionDialog() {
  ...
  const incomingPending = !_isCliVsCli() && isTakebackPending() && pending.requester === "llm";
...
function renderLastMove() {
  ...
  elements.lastMoveMeta.textContent = i18n.t("lastMove.meta", {
    actor: _gameActorName(lastMove.actor),
    uci: lastMove.uci,
  });
}
...
function orderedSquares() {
  const blackAtBottom = !_isCliVsCli() && ui.snapshot.human_color === "black";
  const files = blackAtBottom ? ["h", "g", "f", "e", "d", "c", "b", "a"] : ["a", "b", "c", "d", "e", "f", "g", "h"];
...
function renderBoard() {
  ...
  orderedSquares().forEach((square, index) => {
    ...
    if (piece) {
      ...
      glyph.className = `piece-glyph ${piece === piece.toUpperCase() ? "piece-glyph-white" : "piece-glyph-black"}`;
...
function chooseMove(candidates) {
  ...
  candidates.forEach((move) => {
    ...
    const promotionColor = ui.snapshot.human_color;
    ...
    glyph.className = `promotion-glyph piece-glyph ${promotionColor === "white" ? "piece-glyph-white" : "piece-glyph-black"}`;
    glyph.textContent = PIECE_GLYPHS[promotionColor === "white" ? promotion.toUpperCase() : promotion];
...
async function submitTakeback(action) {
  if (_isCliVsCli() || ui.requestPending) {
    return;
  }
...
/** @description 선택한 대국 방식과 참가자 정보로 게임을 시작합니다. */
async function startGame(gameMode, options) {
  ui.requestPending = true;
  ui.error = null;
  render();
  const request = gameMode === "cli_vs_cli"
    ? {
      game_mode: gameMode,
      white_name: options.whiteName,
      black_name: options.blackName,
    }
    : {
      game_mode: "human_vs_llm",
      human_color: options.humanColor,
    };
  try {
...
function bindEvents() {
  ...
  elements.colorButtons.forEach((button) => {
    button.addEventListener("click", () => startGame("human_vs_llm", { humanColor: button.dataset.color }));
  });
  elements.modeButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ui.setupMode = button.dataset.gameMode;
      ui.error = null;
      render();
    });
  });
  elements.cliPlayerForm.addEventListener("submit", (event) => {
    event.preventDefault();
    startGame("cli_vs_cli", {
      whiteName: elements.cliWhiteInput.value.trim() || null,
      blackName: elements.cliBlackInput.value.trim() || null,
    });
  });
  elements.localeSelects.forEach((select) => {
